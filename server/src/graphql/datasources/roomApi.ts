...
export default class roomApi<TData> extends MongoDataSource<TData> {
            ...
            community.rooms.forEach((room) => {
                if (room.name === name)
                    throw new ApolloError(
                        'You already have a room with that name, please choose a unique name',
    ...
    async deleteRoom(communityId: string, roomId: string) {
        ...
        try {
            ...
            let community: any = await communityApi.verifyCommunityExists(
                communityId
            );
            ...
            if (roomIds.indexOf(roomId) === -1) {
                throw new ApolloError(
                    'Failed deletion. Room does not exist!',
                    errorCodes.roomNotFound
                );
            }
