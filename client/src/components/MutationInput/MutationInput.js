...
const MutationInput = ({
    ...
    const handleSubmit = async () => {
        try {
            ...
            let variableObj = customVariables
                ? Object.assign({}, inputVarObj, ...customVariables)
                : inputVarObj;
    ...
    return (
        <div className={cx('_modalSection')}>
            <div className={cx('_sectionLabel')}>{dataTitle}</div>
            <input
                maxLength={maxLength}
                value={text}
            ...
            <div className={cx('info')}>
                {errorState && <div className={cx('error')}>{errorState}</div>}
